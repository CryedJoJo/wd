...
1、代码记不住怎么办，八股文背了又忘怎么办？
```
代码如果真要硬背下来是可以背下来的，但是长期来看也是会忘记的，八股文也是一样。
所以，第一点，功利来讲代码和八股文可以在面试的那段时间高压背一下，过后不用太在意遗忘这个问题；第二点，从个人职业发展或者知识学习的角度出发记得住xxx代码第一行是什么、第二行是什么远远没有知道这个代码是为了解决什么问题、这个代码这样设计的初衷是什么重要。并且，从记忆的层面看，记住一个问题产生、发展、解决的流程比记住代码本身的实现更关键、也更容易些。

比方说，我现在面试背进程池的代码和框架，我该怎么背呢？网上找一份成百上千行的代码，一行一行的背吗？显然不是明智之举，这是一个比较低效的方式。
按我之前说的，我可以通过问题解决和代码设计初衷的角度去思考记忆。大致分为如下阶段：

1、我们一开始需要能读得懂这份进程池的代码。首先你得读得懂，不然你背半天，连代码的含义都不明白还搞什么呢？

2、读懂代码之后，别急着背，可以先对代码进行分模块，搞清楚每个模块的代码实现了什么功能。

3、之后分析下为什么需要这个模块，这个模型实现的功能没有行不行？有没有其它的实现方式？这时可以问问ai，相关的问题轻易就能知道答案。

4、最后汇总一下，画一下模块间的关系图，进而得到一份由功能模块构成的进程池框架，这样你可以通过这个框架 完整知道这份代码的设计逻辑，和每个模块的设计初衷。如果ai问得好的话，你还能从原有代码蓝本的实现上了解一些设计上不同的实现思路，达到举一反三的效果。

这样学习记忆代码的话，是从问题因果关系的角度去记忆，更像是在了解一个故事。下面举个真实的例子：

1、我一开始有一个在同一台linux上进行消息发送的代码，我现在想让它运行在不同的机器上，现在怎么办？
答案：所以我引入了tcp/ip，进而我代码里面自然而然就使用了socket，然后我就去记忆socket大致怎么使用；

2、然后我想要我的程序能运行在很多linux机器上，他们都可以同时和我的机器进行通信，现在怎么办？
答案：所以我使用了select，然而select没有epoll好，我又改用了epoll（这儿为什么不用select，改用epoll也是一个问题的产生和解决过程。select不好在哪儿，epoll好在哪儿的问题）实现io多路复用，来实现大量链接的问题；

3、我不想让其他所有机器只能和一台机器通信，这种多对一的通信方式。我想实现多对多的通信，现在怎么办？
答案：所以，我使用了cs模式，让client都发信息给我主机上的server程序，server在统一转发个每个client。继而，我又由此在server端引入了进程池框架，让一个进程负责接收client的连接请求，其他几个进程负责维持链接的消息发送；

4、...

这样记忆一个程序框架要比单纯背代码要牢靠深刻！也更有利于我们学习。这样了解到的不只是代码，还有程序设计的思想和解决问题的思路。
```
...
> 虚拟CPU和虚拟内存的引入保证了进程的隔离性。进程间通信（IPC **Inter-process communication**）用于跨越这种隔离，将数据或事件在多个进程间传递
...
#### 异步拉起同步 （一个单独的线程首信号，进程间通信方式再将信息发给另一个进程，这个进程控制了线程池，这样这个进程在收到信息后，就能有时间处理退出逻辑）

# 2、C++ base

C++ 中，关键字是语言保留字，运算符是用于表达式计算的符号或关键字；表达式是可求值结构，语句是程序执行单位。表达式本身不等于语句，但表达式后加分号可以构成表达式语句，而语句范围更大，包含表达式语句、流程控制语句和跳转语句等。
...
c++中的 const char * 才是c中的char *
c++中 不加const 修饰的化，字符串是能够被修改的 
...
从 C++ 语言语义上，引用不是指针，而是对象的别名/绑定；从编译器实现上，引用常常借助地址来实现，所以底层效果很多时候像一个不可为空、不可改绑、自动解引用的受限指针。但不能简单地说“引用本质就是指针”，也不能绝对地说“引用一定不占内存”或“一定占一个指针大小”。这些要看具体场景和编译器实现。

#### 引用指针使用对比

- 第一，语义上不同：
指针是一个保存地址的对象，而引用是某个对象的别名。引用必须在定义时绑定到一个对象，之后不能改绑；指针可以后续再赋值，也可以改指向别的对象。

- 第二，使用方式不同：
指针访问目标对象时需要显式解引用 *p，引用使用起来和原对象几乎一样，直接写引用名即可。

- 第三，空值语义不同：
指针可以为 nullptr，适合表达“可能没有对象”；引用在语言语义上要求绑定一个有效对象，不用来表达空值。

- 第四，应用场景不同：
引用最常见的用途是函数参数传递，尤其是 T& 用于修改实参，const T& 用于避免大对象拷贝并保持只读；
指针更适合表达可空、可重定向、链式结构、底层内存操作等语义。

最后，从实现上看，引用通常会借助地址来实现，所以底层行为很多时候像一个受限指针；但从 C++ 语言语义上，引用不是指针，而是对象别名。

#### inline

inline 函数为什么必须写在头文件中：因为 内联函数在编译之前就需要在代码中使用内联的地方做文本替换，如果写在.c 里面，需要在编译后 的链接阶段才去找实现，而内联函数需要在编译前就要找到函数，所以必须将inline函数写在.h 里面，让使用内联函数的地方 在编译前 就能找到函数实现。
...
拷贝构造的参数 需要是 className(const className & left)

引用是为了避免 递归调用，如果不写引用 会导致拷贝时递归调用。这样写编译器会直接报错
非const 引用 无法绑定右值。
...
- 临时变量，临时对象时右值，能取地址的是左值，不能取地址的是右值。
...
- 局部对象 在程序离开局部对象的作用域时销毁

- 静态对象 在整个程序结束时销毁

- 堆对象 使用 delete 表达式显示删除对象时销毁
